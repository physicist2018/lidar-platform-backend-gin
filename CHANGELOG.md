...
- **Heatmap SVG: персентильное масштабирование цветовой шкалы** — вместо абсолютного min/max для границ цвета теперь используются 5-й и 95-й персентили. Это обрезает выбросы и улучшает контрастность изображения. Цветовая шкала по‑прежнему blue→cyan→green→yellow→red.
